...
local configDirectory = getWorkingDirectory():gsub('\\','/') .. "/Mining Tool"
local path_settings = configDirectory .. "/settings.json"
local path_helper = getWorkingDirectory():gsub('\\','/') .. "/Mining Tool.lua"
local settings = {}
...
function check_update()
	...
	os.remove(path)
	local url = 'https://github.com/KJLikuur/Scripts-Lua/raw/refs/heads/main/Mining%20Tool/Update_Info.json'
...
imgui.OnFrame(function() return OilMenu[0] end, function(player)
    ...
    imgui.Begin(' Mining Tool', OilMenu, imgui.WindowFlags.NoCollapse)
